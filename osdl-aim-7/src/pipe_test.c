...
static int read_write_close(int, char *title, int rd_fd, int wr_fd);	/* the actual test */

static int count = 0;
static int sizes[] = {
	1, 3, 5, 7, 16, 32, 64, 512, 1024, 2048, 4096,	/* misc. sizes */
	1, 3, 5, 7, 16, 32, 64, 512, 1024, 2048, 4096,
...
double xy[100][2];		/* holds xy pairs */

int fflag = 0;			/* completion flag */

struct sockaddr_in rd_in,	/* read socket */
 wr_in;				/* write socket */

struct hostent
*hp;

char myname[1024];		/* hostname placed here */
...
static int tcp_test();
static int udp_test();
static int fifo_test();
static int stream_pipe();
static int dgram_pipe();
static int pipe_cpy();

source_file *pipe_test_c()
{
	...
	if (hp == NULL) {	/* handle errors */
		fprintf(stderr,
			"pipe_test_c(): couldn't gethostbyname() using <%s>, errno = %d\n",
			myname, errno);
		exit(-1);
	}
	 ...
	 * register the tests
	 */
	register_test("shared_memory", 1, "DISKS", shared_memory, 100,
		      "Shared Memory Operations");
	/* CAUTION: update '100' in both places in shared_memory() if change here */
	register_test("tcp_test", 1, "90", tcp_test, 90,
		      "TCP/IP Messages");
	register_test("udp_test", 1, "100", udp_test, 100,
		      "UDP/IP DataGrams");
#ifdef NO_SOCKETPAIR
	register_test("stream_pipe", 1, "100", tcp_test, 100,
	...
	register_test("pipe_cpy", 1, "100", pipe_cpy, 100,
		      "Pipe Messages");
	register_test("fifo_test", 1, "100", fifo_test, 100,
		      "FIFO Messages");
...
static int tcp_test(int argc, char **argv, struct Result *res)
{
...
static int udp_test(int argc, char **argv, struct Result *res)
{
   ...
   * FIFO test
   **********************************************************/
static int fifo_test(int argc, char **argv, struct Result *res)
{
	int status, wr, rd;
	int i64;
	char fn1[STRLEN];
	 ...
	 */
	if (sscanf(*argv, "%d", &i64) < 1) {
		fprintf(stderr, "fifo_test(): needs 1 argument!\n");
		return -1;
	}
	(void) sprintf(fn1, "./%s", FN1);	/* create file name */
	mktemp(fn1);	/* generate new file name */
	unlink(fn1);		/* make sure files are gone */
	status = mkfifo(fn1, 0666);	/* create the FIFO */
	if (status < 0)
		FATAL("Creating fifo");	/* if error, stop */
   ...
   * Stream Pipes
   **********************************************************/
static int stream_pipe(int argc, char **argv, struct Result *res)
{
	int status, pipefd[2];
	int i64;
   ...
   * Datagram Pipes
   **********************************************************/
static int dgram_pipe(int argc, char **argv, struct Result *res)
{
	int status, pipefd[2];
	int i64;
   ...
   * Pipes
   **********************************************************/
static int pipe_cpy(int argc, char **argv, struct Result *res)
{
	int status, pipefd[2];
	int i64;
...
static int writen(int fd, char *buf, int size)
{
	int count, total, result;
...
static int read_write_close(int loops, char *title, int rd_fd, int wr_fd)
{				/* the actual test */
	...
	while (--loops > 0) {
		...
		if (result != size) {
			fprintf(stderr, "Write returned %d -- %s, errno = %d\n",
				result, title, errno);
			perror("pipe write");
			return (-1);
		}
 ...
 */
static void create_shared_memory(int *sem, int *shmid)
{
...
static int shared_memory(int argc, char **argv, struct Result *res)
{
