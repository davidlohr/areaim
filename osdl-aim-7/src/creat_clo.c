...
**                                                             **
****************************************************************/
/* We are using tmpnam now 
 * 03/13/03 Cliff White 
 */
...
static int /* the tests in this file */ creat_clo(),	/* create and then close files */
...
source_file *creat_clo_c()
{
	static source_file s = { " @(#) creat_clo.c:1.17 3/4/94 17:19:34",	/* SCCS info */
		__FILE__, __DATE__, __TIME__
	};

	register_test("creat_clo", 1, "DISKS", creat_clo, 1000,
		      "File Creations and Closes");
	register_test("page_test", 1, "100", page_test, 1700,
		      "System Allocations & Pages");
	register_test("brk_test", 1, "1000", brk_test, 17000,
		      "System Memory Allocations");
	register_test("jmp_test", 1, "1000", jmp_test, 1000,
		      "Non-local gotos");
	register_test("signal_test", 1, "1000", signal_test, 1000,
		      "Signal Traps");
	register_test("exec_test", 1, "5", exec_test, 5, "Program Loads");
	register_test("fork_test", 1, "100", fork_test, 100,
		      "Task Creations");
	register_test("link_test", 1, "DISKS", link_test,
...
static int signal_test(int argc, char **argv, struct Result *res)
{
	int i32,		/* loop count */
 ...
 */
static void dummy_function(jmp_buf buf, int count)
{
	if (count == 0)
		longjmp(buf, 1);
...
static int jmp_test(int argc, char **argv, struct Result *res)
{
	static int /* static so they'll survive */ i32,	/* number of iterations */
...
static int link_test(int argc, char **argv, struct Result *res)
     /*
      ...
      */
{
	...
	char buffer0[1024],	/* filename buffers */
	 buffer1[1024],
	    buffer2[1024],
	    buffer3[1024],
	    buffer4[1024],
	...
	if (argc != 1) {
		fprintf(stderr, "test requires one arg\n");
	}
	/*
...
#ifndef hpux
		mktemp(fns[i]);	/* force it to be unique (use our function) */
#else
		/* HP-UX generates the same name over and over since
		 * the file doesn't exist (keeps reusing the pid).
		 */
...
static int page_test(int argc, char **argv, struct Result *res)
{
	int i,			/* loop variable */
...
static int brk_test(int argc, char **argv, struct Result *res)
{
	int i, n, i32, brk();
	void
	*oldbrk, *newbrk, *sbrk();
...
static int exec_test(int argc, char **argv, struct Result *res)
{
	int fval, status, n, i32;
	...
	while (n--) {
		...
		if (fval == 0) {	/* we're the child */
			char	tmp[1024];
			sprintf(tmp, "%s/true", SCRIPTLOCATION);
			status = execl(tmp, "true", NULL);
			perror("\nexec_test");	/* tell more info */
			fprintf(stderr,
				"Cannot execute %s/true' (status = %d)\n",
				tmp, status);
...
static int fork_test(int argc, char **argv, struct Result *res)
{
	int fval, status, n, i32;
...
#ifdef __hpux
// For legacy reasons, stdio.h on HP-UX defines FILENAME_MAX to be 14.
// This causes the code here to corrupt random locations on the stack,
// since the fn4 array is allocated on the stack and filenames created
// here are sometimes more than 14 characters.  This has undesirable
// effects.  It only took about 1 day to figure this one out...
#undef FILENAME_MAX
#define FILENAME_MAX 4096
#endif // __hpux

static int creat_clo(int argc, char **argv, struct Result *res)
{				/* does the creat/close loop */
	...
	j = 0;			/* make gcc happy */
	if (argc != 1) {
		fprintf(stderr, "Test requires 1 argument\n");
	}
	if (**argv)		/* if directory passed */
		sprintf(fn4, "%s/%sXXXXXX", *argv, TMPFILE2);	/* append values */
	else			/* no directory passed */
		sprintf(fn4, "%sXXXXXX", TMPFILE2);	/* copy the filename over */
	mktemp(fn4);		/* create unique string */
