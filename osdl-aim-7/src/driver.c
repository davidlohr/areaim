...
#ifndef hpux
/* MARCIA: I have getopt_long source code on mammoth - do we what to bring it in? */
		case 0:
			if (opt_num == 1) {
				t_minu = atoi(optarg);
			} else if (opt_num == 2) {
...
/* MARCIA - DAN z: pass config file, Z: pass tool/script name (default perf_tools.sh) */
	...
	if (no_syncs) {
		printf("Not executing sync during disk activity---faster execution.\n");
	} else {
		printf("Using default sync behavior during disk activity;  consider using -y.\n");
	}
...
int multiuser(struct input_params *invars, int totalhits)
{
	...
	int adapt_cnt = 0;
	double mjobs[5]; 	/* MARCIA: fixed index. It was 5 */
				// mjobs[5] is never used (was initialized).
				// An array size of 5 is correct;
				// initializing mjobs[5] was wrong.
...
int runloop(struct _aimList *tlist, struct runloop_input *rl)
{
	...
	/* MARCIA */
	int tools_on = 0;
	/* DAN: Start perf monitoring tools. If ENV is in a file, filename is passed in -z command line 
 	 * and available here in tool_env
	 */
        if (do_tools) {
		char action[256];
		sprintf(action, "%s setup %d %s", tool_name, rl->runnum, tool_env);
		if ( system(action) ) {
			printf("ERROR with [%s]\n", action);
			exit(1);
		}
...
#define CORRECTION
#ifdef CORRECTION
/* delta is in milliseconds (returned by timedelta)
 * start_tick is in CLK_TCKs they dont change.
 * delta -> seconds: delta/1000 as is done every where else
 */
				sprintf(dbug_msg,
					"pid %d CUload:%d Test:%s delta:%ld st:%ld seconds  %-8.2f\n",
		 ...
		 */
                if (do_tools) {
			char action[256];
			sprintf(action, "%s begin %d %s", tool_name, rl->runnum, tool_env);
			if ( system(action) ) {
				printf("ERROR with [%s]\n", action);
				exit(1);
			}
			...
			if ((dead_kid > 0) && (kids_forked > 0)) {
				if (WIFEXITED(status)) {
					...
					/* DAN: Start perf monitoring tools. If ENV is in a file, filename is passed in -z command line 
			 		 * and available here in tool_env
					 */
					if (tools_on && (num_children > (rl->runnum)/8)){
						char action[256];
						sprintf(action, "%s halt %d %s", tool_name, rl->runnum, tool_env);
						if ( system(action) ) {
							printf("ERROR with [%s]\n", action);
							exit(1);
						}
		 ...
		 */
        	if (do_tools) {
			char action[256];
			sprintf(action, "%s report %d %s", tool_name, rl->runnum, tool_env);
			if ( system(action) ) {
				printf("ERROR with [%s]\n", action);
				exit(1);
			}
...
#ifdef hpux
	/* on hpux "times" does not accept NULL */
...
long timedelta(long start_time)
{
	...
	long hz = get_mhertz();
#ifdef hpux
	/* hpux returns -1 if pass in NULL */
	struct tms buf;
	stamp = times(&buf);
#else
	stamp = times(NULL);
